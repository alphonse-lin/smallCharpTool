...
namespace UrbanX.Planning.UrbanDesign
{
    public class BuildingGeometry : IDisposable
    {
            ...
            ref double totalFootPrintAreas, ref double totalBuildingAreas)
        {
            ...
            BuildingType bottomType, towerType, towerType1;
            ...
            if (bottomArea * bottomFloor > mixArea)
            {
                ...
                towerType = new BuildingType(buildingType.TypeName, new int[] { 0, bottomFloor }, buildingType._siteArea);
                towerType1 = new BuildingType(buildingType.TypeName, new int[] { 0, towerFloor - bottomFloor }, buildingType._siteArea);
            }
            else
            {
                ...
                {
                    towerType = new BuildingType(buildingType.TypeName, new int[] { mixTowerFloor, bottomFloor - mixTowerFloor }, buildingType._siteArea);
                    towerType1 = new BuildingType(buildingType.TypeName, new int[] { 0, towerFloor - bottomFloor }, buildingType._siteArea);
                }
                else
                {
                    towerType = new BuildingType(buildingType.TypeName, new int[] { bottomFloor, 0 }, buildingType._siteArea);
                    towerType1 = new BuildingType(buildingType.TypeName, new int[] { mixTowerFloor - bottomFloor, towerFloor - mixTowerFloor }, buildingType._siteArea);
                }
            }
            ...
            if(towerType.Floors.Sum() > 0)
            {
                brepsList.AddRange(GetBreps(towerOutline, towerType, out string[] brepFunctions1, out Curve[] brepOutlines1, out double[] brepHeights1, out int[] brepFloors1, out double[] brepAreas1,
                    out Curve[] layers1, out Curve roofOutline1, out double footPrintArea1, out double totalBrepsArea1, true));

                brepFunctionsList.AddRange(brepFunctions1);
                brepOutlinesList.AddRange(brepOutlines1);
                brepHeightsList.AddRange(brepHeights1);
                brepFloorsList.AddRange(brepFloors1);
                brepAreasList.AddRange(brepAreas1);

                layerlist.AddRange(layers1);

                roofOutlinesList.Add(roofOutline1);
                floorsList.Add(layers1.Length);

                totalFootPrintAreas += footPrintArea1;
                totalBuildingAreas += totalBrepsArea1;

                if (towerType1.Floors.Sum() > 0)
                {
                    // Add tower upper breps.
                    brepsList.AddRange(GetBreps(roofOutline1, towerType1, out string[] brepFunctions2, out Curve[] brepOutlines2, out double[] brepHeights2, out int[] brepFloors2, out double[] brepAreas2,
                            out Curve[] layers2, out Curve roofOutline2, out double footPrintArea2, out double totalBrepsArea2));

                    brepFunctionsList.AddRange(brepFunctions2);
                    brepOutlinesList.AddRange(brepOutlines2);
                    brepHeightsList.AddRange(brepHeights2);
                    brepFloorsList.AddRange(brepFloors2);
                    brepAreasList.AddRange(brepAreas2);

                    layerlist.AddRange(layers2);

                    roofOutlinesList.Add(roofOutline2);
                    floorsList.Add(layers2.Length);

                    totalFootPrintAreas += footPrintArea2;
                    totalBuildingAreas += totalBrepsArea2;
                }
            }        
        }
