    ...
    public class DesignCalculator : IDisposable
    {
        ...
        public DesignResidential CalculateResidentialTypes(int cityIndex, double radiant, double[] scores, ResidentialStyles residentialStyle, double tolerance, out Curve NRsite ,out double NRFar)
        {
            ...
            else if (_mixType == MixTypes.Vertical)
            {
                ...
                var upperFloor = (int)Math.Round(resident.BuildingFloors[1] * (1 - _mixRatio));
        ...
        private void CorrectResitentialDensity(int cityIndex , ref double far, ref double density, out int initialFloors)
        {
        ...
        private void CorrectResitentialDensity1(int cityIndex,Curve site,double radiant,  ref double far, ref double density, out int initialFloors)
        {
            ...
            // Far max: f< (tkx2+lt)/(lkx+lt).
            var fmax = (t * k * Math.Pow(floorUpperLimit, 2) + l * t) / (l * k * floorUpperLimit + l * t);
        ...
        //private BuildingType[] CreateSiteBuildingTypes(string[] buildingTypes, int[] counts, int[] floors)
        //{
        //    LinkedList<BuildingType> result = new LinkedList<BuildingType>();

        //    for (int i = 0; i < buildingTypes.Length; i++)
        //    {
        //        BuildingType type = new BuildingType(buildingTypes[i], new int[] { 0, floors[i] }, _siteArea);
        //        var count = counts[i];
