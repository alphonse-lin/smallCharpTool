...
namespace UrbanX.Planning.UrbanDesign
{
    public static class DesignToolbox
    {
        ...
        private static Point3d[] GetMidPoints(Curve[] allRoads)
        {
        ...
        public static void SplitRecursive(Curve curve, BSPTreeNode node, double[] currentScores, double radiant, bool renewRadiant, double tolerance, ref LinkedList<Curve> result, ref LinkedList<double> radiants, ref LinkedList<double[]> scores, ref LinkedList<int> nodeKeys )
        {
            ...
            else
            {
                if (node.HasOnlyLeftChild())
                {
                    ...
                    {
                        leftRadiant = GetRadiant(curve, tolerance);
                    }
                    else
                    {
                        leftRadiant = radiant;
                ...
                else
                {
                    ...
                    // TODO: Check currentScores' order. Because new radiant will create a new boundingBox with different line order.
                    ...
                    if (!node.LeftChild.IsLeafNode() && renewRadiant)
                    {
                        leftRadiant = GetRadiant(curves[0],tolerance);
                    }
                    else
                    {
                        leftRadiant = radiant;
                    ...
                    {
                        rightRadiant = GetRadiant(curves[1],tolerance);
                    }
                    else
                    {
        ...
        private static double GetRadiant(Curve site, double tolerance)
        {
            // Get control polygon count.
            var polyline = ConvertToPolyline(site, tolerance);
            ...
            var radiant = Math.Acos(Math.Max(Math.Min(v * Vector3d.XAxis, 1.0), -1.0));
        ...
        /// <returns></returns>
        public static DesignResult[] ComputingDesign(SiteParameters[] siteParametersInput, int cityIndex, double tolerance, out List<Line> edgesd, out List<double> scoresd)
        {
