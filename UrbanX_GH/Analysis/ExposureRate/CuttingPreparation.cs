...
namespace AnalysisSpatial_Exposure
{
    /// <summary>
    ...
    public class CuttingPreparation
    {
        ...
        public CuttingPreparation(List<Line> curves, List<Point3d> pts, bool normalized = true, double radius = 500, int stepInDegree = 10)
        {
            _rays = DrawRays(pts, radius, stepInDegree);
        ...
        private void Normalized(bool flag)
        {
            if (flag)
            {
                ...
                //foreach (var item in Outlines.Keys)
                //{
                //    var times = Outlines[item];
                //    var ptg = Math.Round(times / item.Length,4);

                //    OutlinesNormilized.Add(item, ptg);
                //}
            }
            return;
        }

        //private static bool LineSegementsIntersect(Line line1, Line line2, out Vector3d intersection, bool considerCollinearOverlapAsIntersect = false)
        //{
        //    intersection = new Vector3d();

        //    Vector3d p = new Vector3d(line1.From);
        //    Vector3d p2 = new Vector3d(line1.To);

        //    Vector3d q = new Vector3d(line2.From);
        //    Vector3d q2 = new Vector3d(line2.To);
